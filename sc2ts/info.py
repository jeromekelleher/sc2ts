...
class CopyingTable:
    def __init__(
        ...
        using the ``IPython.display.HTML`` function. The table is accessible from the
        ``.html`` attribute
        ...
        """
        def node_mutations(ts, node):
            muts = {}
            for mut_id in np.where(ts.mutations_node == node)[0]:
        ...
        if edges is None:
            # the required edge table wasn't given, so recalculate
        ...
        variants = ts.variants(samples=samples, isolated_as_missing=False)
        mutations = node_mutations(ts, node)
...
class TreeInfo:
    ...
    def copying_table(self, node, edges=None, **kwargs):
        return CopyingTable(self.ts, node, edges=edges, **kwargs).html
